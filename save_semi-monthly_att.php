...
function calculateBasicSalary($dailyRate, $paySchedule)
{
    switch (strtolower($paySchedule)) {
        ...
        case 'fixed':
        default:
            return $dailyRate * 30; // 15 days per fixed period
    }
}
...
try {
    foreach ($groupedByEmployee as $employeeName => $entries) {
        ...
        if (strtolower($paySchedule) !== 'semi-monthly') {
            // Skip non-semi-monthly employees in this file
            continue;
        }
        ...
        // Group entries into semi-monthly chunks (7-day windows)
        $weekGroups = [];
        $currentGroup = [];
        $groupStartDate = new DateTime($entries[0]['date']);
        $groupEndDate = clone $groupStartDate;
        $groupEndDate->modify('+15 days');
        ...
        // Process each semi-monthly group
