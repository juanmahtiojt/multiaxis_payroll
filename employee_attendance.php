...
// Log page access
log_activity($conn, $username, 'Accessed employee attendance page', $current_page);

// Log attendance upload if redirected from upload_excel.php
if (isset($_SESSION['attendance_uploaded']) && $_SESSION['attendance_uploaded'] === true) {
    $uploadedRecords = $_SESSION['uploaded_records_count'] ?? 0;
    log_activity($conn, $username, "Uploaded {$uploadedRecords} attendance records", $current_page);
    unset($_SESSION['attendance_uploaded']);
    unset($_SESSION['uploaded_records_count']);
}

// Log attendance deletion if redirected from clear_attendance.php
if (isset($_SESSION['attendance_deleted']) && $_SESSION['attendance_deleted'] === true) {
    log_activity($conn, $username, "Deleted all attendance records", $current_page);
