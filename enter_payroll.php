...
if (isset($_GET['id_no'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM daily_rate WHERE id_no = ? AND pay_schedule = 'fixed'");
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="nav-section">
            ...
            <a href="enter_payroll.php" class="<?php echo ($current_page == 'enter_payroll.php') ? 'active' : ''; ?>">
        ...
        <div class="container-box">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    ...
                    $result = $conn->query("SELECT * FROM daily_rate WHERE pay_schedule = 'fixed'");
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<tr>
                                ...
                                <td>
                                    <form action='enter_payroll.php' method='get' style='margin: 0;'>
                                        <input type='hidden' name='id_no' value='" . $row['id_no'] . "'>
                                        <button type='submit' class='btn btn-sm btn-primary'>
                                            <i class='fas fa-edit'></i> Enter Payroll
                                        </button>
                                    </form>
                                </td>
                            </tr>";
                ...
                <form action="submit_payroll.php" method="post">
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="end_date" required class="form-control" onchange="checkHolidays()">
                        </div>
                        ...
                        $pay_fields = [
                            "hours" => "Regular OT Hours",
                            "rate" => "Regular OT Rate",
                            "regular_ot" => "Total Regular OT",
                            "sunday_hours" => "Sunday OT Hours",
                            "sunday_rate" => "Sunday Rate",
                            "rest_day_pay" => "Total Sunday OT Pay",
                            "special_holiday_pay" => "Special Holiday Pay",
                            "thirteenth_month" => "13th Month Pay",
                            "overtime_pay" => "TOTAL OT PAY"
                        ];

                        foreach ($pay_fields as $field => $label) {
                            if ($field === 'rate') {
                                // Fixed regular OT rate, readonly with preset value and id="rate"
                                echo "<div class='form-group'>
                                        <label>$label</label>
                                        <input type='number' id='rate' name='$field' step='0.01' readonly value='$fixed_regular_rate' class='form-control'>
                                    </div>";
                            } elseif ($field === 'sunday_rate') {
                                // Fixed Sunday OT rate, readonly with preset value and id="sunday_rate"
                                echo "<div class='form-group'>
                                        <label>$label</label>
                                        <input type='number' id='sunday_rate' name='$field' step='0.01' readonly value='$fixed_sunday_rate' class='form-control'>
                                    </div>";
                            } elseif ($field === 'hours') {
                                // Editable Regular OT hours, id="hours"
                                echo "<div class='form-group'>
                                        <label>$label</label>
                                        <input type='number' id='hours' name='$field' step='0.01' min='0' value='0' class='form-control' required>
                                    </div>";
                            } elseif ($field === 'sunday_hours') {
                                // Editable Sunday OT hours, id="sunday_hours"
                                echo "<div class='form-group'>
                                        <label>$label</label>
                                        <input type='number' id='sunday_hours' name='$field' step='0.01' min='0' value='0' class='form-control' required>
                                    </div>";
                            } elseif (in_array($field, ['regular_ot', 'rest_day_pay', 'overtime_pay'])) {
                                // Calculated fields, readonly with proper ids
                                $id = $field;  // id attribute same as name
                                echo "<div class='form-group'>
                                        <label>$label</label>
                                        <input type='number' id='$id' name='$field' step='0.01' readonly value='0.00' class='form-control'>
                                    </div>";
                            } else {
                                // Editable fields (special_holiday_pay, thirteenth_month)
                                echo "<div class='form-group'>
                                        <label>$label</label>
                                        <input type='number' id='$field' name='$field' step='0.01' value='0.00' class='form-control'>
                                    </div>";
                            }
                        }
