    ...
    foreach ($groupedByEmployee as $employeeName => $entries) {
        ...
        switch (strtolower($paySchedule)) {
            case 'weekly':
                ...
                // Sort entries by date (already sorted)
                $allDates = array_column($entries, 'date');
                sort($allDates);

                // Group entries into chunks of max 7 days, allowing gaps (e.g., skip Sundays if needed)
                $weekGroups = [];
                $currentGroup = [];
                $groupStartDate = new DateTime($allDates[0]);
                $groupEndDate = clone $groupStartDate;
                $groupEndDate->modify('+6 days'); // 7-day window
                ...
                // Now process each week group separately
                foreach ($weekGroups as $weekEntries) {
                    // Calculate start and end date for this group
                    $startDate = $weekEntries[0]['date'];
                    $endDate = $weekEntries[count($weekEntries) - 1]['date'];

                    // Reset totals for this group
                    $totalWage = 0;
                    $totalUndertime = 0;
                    $totalOvertime = 0;
                    $totalDaysPresent = 0;
                    ...
                    // Process each entry in this week group (your existing per-entry processing code)
                    ...
                    if ($exists > 0) {
                        $errorCount++;
                        // error_log("Skipped duplicate payroll record for employee: {$employeeName} for period {$startDate} to {$endDate}");
                        $duplicateMessages[] = "Skipped duplicate payroll record from period {$startDate} to {$endDate} already in the database";
                        continue; // Skip insertion for this employee and pay period
                    }
